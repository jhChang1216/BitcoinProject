...
def op_equal(stack):
    if len(stack) < 2:
        return False
    element1 = stack.pop()
    print('e1 is ', element1.hex())
    element2 = stack.pop()
    print('e2 is ', element2.hex())
...
def op_checksig(stack, z):
    ...
    try:
        ...
        return False
    if Verify.sig_verify(sig, z, point):
        stack.append(encode_num(1))
    else:
        stack.append(encode_num(0))
    return True
...
def op_checkmultisig(stack, z):
    ...
    der_signautures = []
    for i in range(m):
        der_signautures.append(stack.pop()[:-1])
    stack.pop()
    try:
        points = [S256Point.parse(sec) for sec in sec_pubkeys]
        sigs = [Signature.parse(sig) for sig in der_signautures]
        for sig in sigs:
            if len(points) == 0:
                return False
            while points:
